...
use crate::state::{MinterData, TokenInfo,
    TOTAL_BALANCES, BALANCES, LOGO, MARKETING_INFO, TOKEN_INFO, MINT_TIME,
...
pub fn instantiate(
    ...
    // let lock_duration = Vec::from([1, 3, 6, 12]);
    // LOCK_TIME.save(deps.storage, &lock_duration);
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::Mint { recipient, amount, duration } => execute_mint(deps, env, info, recipient, amount, duration),
        ...
        // ExecuteMsg::UpdateDuration {duration} => execute_update_lock_durations(deps, env, info, duration),
    }
}
...
pub fn execute_mint(
    ...
    let prev_total_balances = TOTAL_BALANCES.may_load(deps.storage, duration)?
    .unwrap_or_default();

    let mut prev_balances: Uint128 = Uint128::from(0u128);
    let user_info = MINT_TIME.may_load(deps.storage, (&rcpt_addr, duration))?
        .unwrap_or_default();
    
    // adding and calculating user total balance of one duration
    if user_info.mint_time > 0u64
    {
        let month_seconds = MONTH_SECONDS * duration;
    ...
    // let balances: StdResult<Vec<(Addr ,Uint128)>> = BALANCES.range(deps.storage, None, None, Order::Ascending).collect();
    // println!("balance {:?}", balances);
    // let lock_info: String = rcpt_addr.to_string() + "_" + &duration.to_string();
    // let mut lock_duration: Vec<(u64, u64)> = Vec::new();
    // lock_duration.push((duration, _env.block.time.seconds()));
    println!("Mint_balances after: {:?}", prev_balances);
    let user_info = UserInfo{
        balance: amount + prev_balances,
        mint_time: _env.block.time.seconds(),
...
// pub fn execute_update_lock_durations(
//     deps: DepsMut,
//     _env: Env,
//     info: MessageInfo,
//     duration: u64,
...
//         return Err(ContractError::Unauthorized {});
//     }
//     let mut lock_time = LOCK_TIME.load(deps.storage)?;
//     lock_time.push(duration);
//     LOCK_TIME.save(deps.storage, &lock_time);
//     Ok(Response::new()
//         .add_attribute("duration", "added"))
// }

#[cfg_attr(not(feature = "library"), entry_point)]
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::Balance { address, duration } => to_binary(&query_balance(deps, _env, address, duration)?),
        ...
        QueryMsg::TotalBalance {duration } => to_binary(&query_total_balance(deps, _env, duration)?),
    }
}

pub fn query_balance(deps: Deps, env: Env, address: String, duration: u64) -> StdResult<BalanceResponse> {
    let address = deps.api.addr_validate(&address)?;
    // let mut balance = BALANCES
    //     .may_load(deps.storage, &address)?
    //     .unwrap_or_default();

    let mut total_balances = TOTAL_BALANCES.may_load(deps.storage, duration)?
    .unwrap_or_default();
    ...
    println!("total_balance:{:?}", balance.clone());
        ...
        let user_info = if let Some(user_info) = 
        MINT_TIME.may_load(deps.storage, (&address, duration))?{
            user_info
        } else {
            UserInfo{
                mint_time: 0u64,
        ...
        if user_info.mint_time > 0u64 {
            let month_seconds: u64 = MONTH_SECONDS * duration;
...
pub fn query_total_balance(deps: Deps, env: Env, duration: u64) -> StdResult<BalanceResponse>{
        // let mut total_balances = TOTAL_BALANCES.may_load(deps.storage, duration)?
        // .unwrap_or_default();

        let total_balances = if let Some(total_balances) = 
        TOTAL_BALANCES.may_load(deps.storage, duration)?{
            total_balances
        } else {
            BalanceInfo{
                mint_time: 0u64,
        ...
        if total_balances.mint_time > 0u64 {
            let month_seconds: u64 = MONTH_SECONDS * duration;
            let time_diff:u64 = env.block.time.seconds() - total_balances.mint_time;
            if time_diff < month_seconds{
                let power_time = month_seconds - time_diff;
                let power_time =  REWARD_CALC_UNIT.multiply_ratio(power_time, 1u64);
                let power_time =  power_time.multiply_ratio(1u64, month_seconds);
...
mod tests {
    ...
    use cosmwasm_std::{coins, from_binary, Addr, CosmosMsg, StdError, SubMsg, WasmMsg, Env, Timestamp};
    ...
    fn get_balance<T: Into<String>>(deps: Deps, env: Env, address: T) -> Uint128 {
        query_balance(deps, env, address.into(), 1u64).unwrap().balance
    }

    // fn get_total_balance(deps: Deps, env: Env, duration: u64) -> Uint128 {
    //     query_total_balance(deps, env, duration).unwrap().balance
    // }
    ...
    fn _do_instantiate(
        ...
        // assert_eq!(get_balance(deps.as_ref(),env, addr), amount);
    ...
    mod instantiate {
        ...
        fn basic() {
            ...
            assert_eq!(
                get_balance(deps.as_ref(),env, "addr0000"),
    ...
    fn can_mint_by_minter() {
        ...
        println!("query_total_balance");
        query_total_balance(deps.as_ref(), env.clone(), 1u64);

        assert_eq!(0, res.messages.len());
        // assert_eq!(get_balance(deps.as_ref(),env.clone(), genesis.clone()), amount);
        // assert_eq!(get_balance(deps.as_ref(),env.clone(), winner.clone()), prize);
        let new_env = mock_env_time(env.block.time.plus_seconds(130));


        let res = execute(deps.as_mut(), new_env.clone(), info, msg2).unwrap();
        assert_eq!(0, res.messages.len());
        let new_env = mock_env_time(env.block.time.plus_seconds(200));
        let new_env2 = mock_env_time(env.block.time.plus_seconds(150));

        println!("query_total_balance"); 
        query_total_balance(deps.as_ref(), new_env2.clone(), 1u64);
        assert_eq!(get_balance(deps.as_ref(),new_env.clone(), genesis), amount);
        assert_eq!(get_balance(deps.as_ref(),new_env.clone(), winner.clone()), amount);

        // assert_eq!(get_total_balance(deps.as_ref(),new_env.clone(), 1u64), 0);
    ...
    fn instantiate_multiple_accounts() {
        ...
        assert_eq!(get_balance(deps.as_ref(), env.clone(), addr1), amount1);
        assert_eq!(get_balance(deps.as_ref(),env, addr2), amount2);
    }
    ...
    fn queries_work() {
        ...
        let data = query(
            deps.as_ref(),
            env.clone(),
            QueryMsg::Balance { address: addr1, duration: 1u64 },
        )
        .unwrap();
    ...
    fn transfer() {
        ...
        assert_eq!(get_balance(deps.as_ref(),env.clone(), addr1), remainder);
        assert_eq!(get_balance(deps.as_ref(),env, addr2), transfer);
        assert_eq!(
    ...
    fn burn() {
        ...
        assert_eq!(get_balance(deps.as_ref(),env, addr1), remainder);
        assert_eq!(
            query_token_info(deps.as_ref()).unwrap().total_supply,
            remainder
        );
    ...
    fn send() {
        ...
        let remainder = amount1.checked_sub(transfer).unwrap();
        assert_eq!(get_balance(deps.as_ref(),env.clone(), addr1), remainder);
        assert_eq!(get_balance(deps.as_ref(),env, contract), transfer);
        assert_eq!(
            query_token_info(deps.as_ref()).unwrap().total_supply,
            amount1
        );
