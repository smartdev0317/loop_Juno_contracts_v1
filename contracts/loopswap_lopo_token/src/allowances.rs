...
use crate::state::{ALLOWANCES, BALANCES, TOKEN_INFO, UserInfo};
...
mod tests {
    ...
    use cosmwasm_std::{coins, CosmosMsg, SubMsg, Timestamp, WasmMsg, Env};
    ...
    fn transfer_from_respects_limits() {
        ...
        assert_eq!(
            get_balance(deps.as_ref(), env.clone(), owner.clone()),
            start.checked_sub(transfer).unwrap()
        );
        assert_eq!(get_balance(deps.as_ref(), env, rcpt.clone()), transfer);
    ...
    fn burn_from_respects_limits() {
        ...
        // assert_eq!(
        //     get_balance(deps.as_ref(), env, owner.clone()),
        //     start.checked_sub(transfer).unwrap()
        // );
    ...
    fn send_from_respects_limits() {
        ...
        assert_eq!(get_balance(deps.as_ref(),env, contract.clone()), transfer);
