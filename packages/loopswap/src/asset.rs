...
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema)]
pub struct StakeableInfoRaw {
    pub asset_infos: AssetInfoRaw,
    pub contract_addr: CanonicalAddr,
    pub liquidity_token: CanonicalAddr,
}

impl StakeableInfoRaw {
    pub fn to_normal(&self, api: &dyn Api) -> StdResult<StakeableToken> {
        Ok(StakeableToken {
            liquidity_token: api.addr_humanize(&self.liquidity_token)?.to_string(),
            //reward_token: api.addr_humanize(&self.reward_token)?.to_string(),
            //contract_addr: api.addr_humanize(&self.contract_addr)?.to_string(),
            token: self.asset_infos.to_normal(api)?.to_string(),
            distribution: vec![],
        })
    }
    ...
    pub fn query_pools(
        ...
        let info_0: AssetInfo = self.asset_infos.to_normal(api)?;

        Ok([Asset {
            amount: info_0.query_pool(querier, api, contract_addr)?,
            info: info_0,
        }])
    }
}

//                                           structure to contain the linked distribution tokens with stakeable token
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema)]
pub struct StakeablePairedDistributionTokenInfo {
    pub token: String,
    pub amount: Uint128,
    // pub pair_add: String,
...
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema)]
pub struct StakeableToken {
    pub liquidity_token: String,
    pub token: String,
    pub distribution: Vec<StakeablePairedDistributionTokenInfo>, //     contain the linked distribution tokens
}